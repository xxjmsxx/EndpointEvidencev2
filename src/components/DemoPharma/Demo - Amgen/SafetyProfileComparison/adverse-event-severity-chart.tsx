  ...
  useEffect(() => {
    if (chartRef.current) {
      ...
      if (ctx) {
        ...
        // Updated to match the specified percentages
        // Grade 3+ for Lumykras + Vectibix: 18.4%
        // Grade 3+ for Krazati + Erbitux: 22.1%
        const lumykrasVectibixData = [50.6, 31.0, 15.4, 3.0, 0.0]
        const krazatiErbituxData = [45.9, 32.0, 18.1, 4.0, 0.0]
        ...
        chartInstance.current = new Chart(ctx, {
          ...
          options: {
            ...
            plugins: {
              ...
              title: {
                display: true,
                text: "Adverse Event Severity Distribution by Treatment (Grade 3+ events: Lumykras+Vectibix 18.4% vs Krazati+Erbitux 22.1%, p=0.15)",
                font: {
                  size: 16,
                  weight: "bold",
                },
        ...
        chartInstance.current.draw = function (...args) {
          ...
          meta0.data.forEach((bar, index) => {
            const data = lumykrasVectibixData[index]
            if (data > 0) {
          ...
          meta1.data.forEach((bar, index) => {
            const data = krazatiErbituxData[index]
            if (data > 0) {
              // Only show labels for non-zero values
              ctx.fillStyle = "#333"
              ctx.fillText(`${data}%`, bar.x, bar.y - 5)
